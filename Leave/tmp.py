...
class LeaveListView(ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'export' in request.POST:
            if from_date == '' and to_date == '' and status == '' and employee == '':
                if self.request.user.groups.filter(name='myansrsourcePM').exists():
...
def update_leave_application(request, status):
    ...
    try:
        ...
        leave_email.delay(msg_html, leave_application.user.email, leave_application.apply_to.email)
        leave_application.save()
        # first_name,status,from_date,to_date,comment,action_by,user_mail,pm_mail
