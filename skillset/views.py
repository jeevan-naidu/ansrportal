...
def SkillSet_assign(request):
    if request.method == 'GET':
        ...
        if request.user.groups.filter(name__in=['myansrsourceHR']).exists() or request.user.is_superuser == True:
            ...
            context = skill_set_details(reportee)
        ...
        return render(request, 'skillassign.html', context)

def skill_set_details(reportee):
    user_details = {"name": "", "deisgnation": "", "department": "", "id": "", "doj": "", "skills": ""}
    context = {}
    designation_all = []
    department = []
    ...
    context['lists'] = lists
    context['count'] = count
    context['skills_all'] = skills_all
    context['designation_all'] = designation_all
    ...
    if request.method == 'GET':
        ...
        if request.user.groups.filter(name__in=['myansrsourceHR']).exists() or request.user.is_superuser == True:
            ...
            if not reportee_business_unit:
                ...
                return render(request, 'skillset.html', context)
