...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if os.path.exists(FEED_DIR):
                    with open(FEED_DIR+'maternity.csv', 'r') as csvfile:
...
def insertToDb1(data_file):
    try:
        columns_list = ['emp_id', 'name', 'bereavement_leave', 'maternity_leave',
         'paternity_leave']
        ...
        if user_obj.user:
            for i in range(2,5):
                ...
                obj, created = LeaveSummary.objects.get_or_create(user=user_obj.user, leave_type=leave_type_obj)
                value = data_file[i]
                if not value:
                    value = "0.0"
