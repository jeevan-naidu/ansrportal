from LaptopAvail.models import Laptop
...
def manager(request, roles):
    ...
    avail_active = Laptop.objects.filter(is_active=True,
                                         user__in=reportees_id,
                                         role=roles).exclude(process_status='Completed',
                                                             request_status__in=['Completed',
                                                                                 'Rolled Back'])
    avail_inactive = Laptop.objects.filter(is_active=False,
                                           user__in=reportees_id,
    ...
    if request.user.groups.filter(name="BookingRoomAdmin"):
        avail_active = Laptop.objects.filter(is_active=True,
                                                    role=roles).exclude(process_status='Completed',
                                                                        request_status__in=['Completed',
                                                                                            'Rolled Back'])
        avail_inactive = Laptop.objects.filter(is_active=False, process_status='Completed')
        return avail_active, avail_inactive
    return None