...
def get_severity_count(project, name, template_id):
    return c_get_severity_count(project, name, template_id)
    # s = 0
    # try:
    #     if name.lower() == "s0":
    #         return s
...
def get_defect_density(s1, s2, s3, q_count):
    return c_get_defect_density(s1, s2, s3, q_count)
...
def get_question_count(project):
    return c_get_question_count(project)
    # s = 0
    # try:
...
def is_project_manager(project, user):
    try:
        s = ProjectDetail.objects.filter(Q(deliveryManager=user)| Q(pmDelegate=user), project=project).exists()
...
def get_project_status(project):
    return c_get_project_status(project)
